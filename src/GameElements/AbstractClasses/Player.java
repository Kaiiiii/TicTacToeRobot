...
public abstract class Player {
	...
	protected double w1_doubleOwnRow, w2_doubleEnemyRow, w3_ownFreeRow, w4_enemyFreeRow, w5_interceptEnemyRow,
	...
	protected int[] findBestMove(Board boardState, Player player) {
		int boardDimension = Board.BOARD_DIMENSIONS;
		double highestScore = -Double.MAX_VALUE, mockScore;
		...
		for (int i = 0; i < boardDimension; i++) {
			for (int j = 0; j < boardDimension; j++) {
				...
				mockScore = getPredictedValue(mockBoard);
	...
	protected double getPredictedValue(Board boardState) {
		int[] boardAnalysis = analyzeBoard(boardState);

		return this.w1_doubleOwnRow * boardAnalysis[0] + this.w2_doubleEnemyRow * boardAnalysis[1]
				+ this.w3_ownFreeRow * boardAnalysis[2] + this.w4_enemyFreeRow * boardAnalysis[3]
				+ this.w5_interceptEnemyRow * boardAnalysis[4] + this.w6_interceptedOwnRow * boardAnalysis[5];
	}
	...
	private int[] analyzeHorizontal(Board boardState, int[] boardScore) {
		...
		int boardDimensions = Board.BOARD_DIMENSIONS, ownCount, enemyCount, emptyCount;
		...
		for (int i = 0; i < boardDimensions; i++) {
			...
			for (int j = 0; j < boardDimensions; j++) {
				if (board[i][j] == this.playerChar)
					ownCount++;
				else if (board[i][j] == Character.MIN_VALUE)
					emptyCount++;
				else
					enemyCount++;
			}
	...
	private int[] analyzeVertical(Board boardState, int[] boardScore) {
		...
		for (int i = 0; i < boardDimensions; i++) {
			...
			for (int j = 0; j < boardDimensions; j++) {
				if (board[j][i] == this.playerChar)
					ownCount++;
				else if (board[j][i] == Character.MIN_VALUE)
					emptyCount++;
				else
					enemyCount++;
			}
	...
	private int[] analyzeDiagonal(Board boardState, int[] boardScore) {
		int[] score = boardScore;
		int boardDimensions = Board.BOARD_DIMENSIONS, ownCount = 0, enemyCount = 0, emptyCount = 0;
		char[][] board = boardState.getGameState();

		for (int i = 0; i < boardDimensions; i++) {
			if (board[i][i] == this.playerChar)
				ownCount++;
			else if (board[i][i] == Character.MIN_VALUE)
				emptyCount++;
			else
				enemyCount++;
		}
		...
		for (int i = 0; i < boardDimensions; i++) {
			if (board[i][boardDimensions - i - 1] == this.playerChar)
				ownCount++;
			else if (board[i][boardDimensions - i - 1] == Character.MIN_VALUE)
				emptyCount++;
			else
				enemyCount++;
		}
