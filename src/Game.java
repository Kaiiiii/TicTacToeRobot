...
public class Game {
	...
	private static Player playGame(Player firstPlayer, Player secondPlayer, Player currPlayer, Board board) throws Exception{
...
//		printBoard(board.getGameState());
		...
		if (winner == firstPlayer.getChar()) return firstPlayer;
		if (winner == secondPlayer.getChar()) return secondPlayer; 
		return null;
	}
	...
	public static void main(String[] args){
		...
		for (int i = 0; i < totalGames; i ++){
			board = new Board();
			winner = currPlayer;
			try{ winner = playGame(firstPlayer, secondPlayer, currPlayer, board);}
			catch(Exception e){
				e.printStackTrace();
				break;
			}
